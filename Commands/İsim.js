...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  message.channel.send(new MessageEmbed().setColor(client.randomColor()).setTitle("Kayıt Sistemi").setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setFooter("ㄓ T Λ U R U S ❤️ AGNES").setTimestamp().setDescription(` Kayıt Olan Kullanıcı: ${uye}\n Verilen Roller: ${uye.roles.cache.filter(a => a.name !== "@everyone").map(x => x).join(', ')}\n Kayıt Eden: ${message.author}\n Güncellenen İsim: \`${yazilacakIsim}\``)).catch();
};
