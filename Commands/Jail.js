...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(uye.voice.channel) uye.voice.kick().catch();
   message.channel.send(embed.setImage("https://cdn.discordapp.com/attachments/713057260108120154/744521936276815892/giphy.gif").setDescription(`${uye} üyesi, ${message.author} tarafından **${reason}** jaile atıldı!\n\n Cezalandırılan Üye: ${uye} \`${uye.user.tag} ^ ${uye.id}\`\n Cezalandıran Yetkili: ${message.author} \`${message.author.tag} ^ ${message.author.id}\`\n Ceza Sebebi: \`${reason}\``)).catch();
  if(ayar.jailLogKanali && client.channels.cache.has(ayar.jailLogKanali)) client.channels.cache.get(ayar.jailLogKanali).send(embed.setImage("https://cdn.discordapp.com/attachments/713057260108120154/744521936276815892/giphy.gif").setDescription(`${uye} üyesi, ${message.author} tarafından **${reason}** nedeniyle Jaile Atıldı!\n\n Ceza ID: \`${uye.id}\`\n Cezalandırlan Üye: ${uye} \`${uye.user.tag} ^ ${uye.id}\`\n Cezalandıran Yetkili: ${message.author} \`${message.author.tag} ^ ${message.author.id}\`\n Ceza Sebebi: \`${reason}\``)).catch();
};
