...
module.exports.onLoad = async (client) => {
    ...
    const levelChat = "778977892653858877"; 
    ...
    client.on("message", async message => {
      ...
      if (nextLevel <= currentlyData.XP) {
        currentlyData.Level++;
        currentlyData.XP = 0;
        let log = client.channels.cache.get(levelChat);
        let embed = new MessageEmbed()
        .setColor("RANDOM")
        .setTitle("LEVEL SİSTEMİ")
        .setDescription(`${message.member}, **tebrikler!** Seviye atladın ve **${currentlyData.Level}. seviyeye** Ulaştın 🎉
`)
        log.send(embed)
...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    let embed = new MessageEmbed().setThumbnail(user.user.avatarURL({dynamic: true, size: 2048})).setAuthor(user.displayName, user.user.avatarURL({dynamic: true, size: 2048})).setFooter("ㄓ T Λ U R U S ❤️ AGNES").setColor(client.randomColor()).setTimestamp();
