...
const sayiEmojiler = {
  0: "<a:sifir:780108161843593256>",
  1: "<a:bir:780108158680957000>",
  2: "<a:iki:780108161750794250>",
  3: "<a:uc:780108161813577768>",
  4: "<a:dort:780108162283470848>",
  5: "<a:bes:780108157706829925>",
  6: "<a:alt:780108153629966356>",
  7: "<a:yedi:780108162384134204>",
  8: "<a:sekiz:780108161830354984>",
  9: "<a:dokuz:780108162601713695>"
};
...
///EMOJİLER
let guild = "780083556994777109";
const bir = client.emojis.cache.find(emoji => emoji.name === "bir");
const iki = client.emojis.cache.find(emoji => emoji.name === "iki");
const uc = client.emojis.cache.find(emoji => emoji.name === "uc");
const dort = client.emojis.cache.find(emoji => emoji.name === "dort");
const bes = client.emojis.cache.find(emoji => emoji.name === "bes");
const alti = client.emojis.cache.find(emoji => emoji.name === "alti");
const yedi = client.emojis.cache.find(emoji => emoji.name === "yedi");
const sekiz = client.emojis.cache.find(emoji => emoji.name === "sekiz");
const dokuz = client.emojis.cache.find(emoji => emoji.name === "dokuz");
const sifir = client.emojis.cache.find(emoji => emoji.name === "sifir");
///EMOJİLER
  ...
  const kanal = "780083557417615389"; // HOŞGELİDNİ ATMASI GEREKEN KANALIN İDSİ
  let user = client.users.cache.get(msg.id);
  require("moment-duration-format");
  const kurulus = new Date().getTime() - user.createdAt.getTime();

  let memberGün = moment(msg.user.createdAt).format("DD");
let memberTarih = moment(msg.user.createdAt).format("YYYY HH:mm:ss");
let memberAylar = moment(msg.user.createdAt).format("MM").replace("01", "Ocak").replace("02", "Şubat").replace("03", "Mart").replace("04", "Nisan").replace("05", "Mayıs").replace("06", "Haziran").replace("07", "Temmuz").replace("08", "Ağustos").replace("09", "Eylül").replace("10", "Ekim").replace("11", "Kasım").replace("12", "Aralık"); 
  ...
  //let gecen = useful.tarih(zaman1)

  var kontrol;
  if (kurulus < 1296000000) kontrol = '`Güvenilir Değil`'
  if (kurulus > 1296000000) kontrol = '`Güvenilir`'
  moment.locale("tr");
 let log = client.channels.cache.get(kanal);
  log.send("<@&780083557367808019>")
  console.log(msg.guild)
 const embed = new MessageEmbed()
 .setAuthor(msg.guild.name, msg.guild.iconURL({ dynamic: true }))
 .setFooter("ㄓ T Λ U R U S ❤️ AGNES")
 .setImage('https://cdn.discordapp.com/attachments/777948793110134786/778334661906137088/d350ea_e2c07df07dad4e5d91142806ad2de20c_mv2.gif')
 .setColor('RED')
 .setDescription(`
 
 **Hoşgeldin! ${msg} Seninle ${üyesayısı} Kişiyiz.** 

 **Sunucumuzun Şuanda Ses Aktifliği ${sessayı} Olarak Görünmektedir.!**
 **Sunucumuza Kayıt Olmak İçin <@&780083557367808019> Yetkilierine Sahip Olan Arkadaşlar Seni Kayıt Etmek İçin Sol Tarafta Bulunan Kayıtsız Odalarda Bekliyor Olacak.**

 **Hesap Bilgilerin**
 Hesap ID : \`${msg.id}\`
 Hesap Katılım Sıralaması : \`${msg.guild.members.cache.sort((a, b) => a.joinedTimestamp - b.joinedTimestamp).map(e => e.id).indexOf(msg.user.id)} / ${msg.guild.memberCount}\`
 Hesap Açılma Süresi : \`${memberGün} ${memberAylar} ${memberTarih}\`
